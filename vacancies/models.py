...
class Vacancy(models.Model):
    ...
    min_educational_level = models.ForeignKey(EducationalLevel, on_delete=models.CASCADE)
    published = models.BooleanField(default=False)
    job_discipline = models.ForeignKey(JobDiscipline, on_delete=models.CASCADE, null=True)
...
class Application(models.Model):
    ...
    work_experience = models.DecimalField(max_digits=5, decimal_places=2, null=True)
    ...
    index = models.PositiveIntegerField(unique=True, editable=False, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.pk:  # Only on creation, not on updates
            max_index = Application.objects.aggregate(models.Max('index'))['index__max']
